...
>(
  ...
  const startCursor =
    nodes.length > 0 ? encodeCursor(nodes[0], options) : undefined;
  const endCursor =
    nodes.length > 0
      ? encodeCursor(nodes[nodes.length - 1], options)
      : undefined;
...
function decodeCursor<Model, Cursor>(
...
function encodeCursor<Model, Cursor>(
  node: Model,
  options: Options<Model, Cursor>,
): string {
